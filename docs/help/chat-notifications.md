...
Titles are sent, because a notification with no subject is not a notification. Put
anything that must not be read by the wrong person in the body, where it stays.

## The one setting that matters

**This channel is the team's own.**

**Ticked**, the channel hears everything: internal issues, incoming reports, and the
fact that somebody left an internal note. This is the normal case — it is your own
Slack — and the box is ticked for you when you add an integration.

**Unticked**, the channel hears only what a client could already see: issues marked
visible to the client, and public comments. Untick it when the channel belongs to a
client, which the per-project filter makes a reasonable thing to want.

The distinction is not decoration. An internal issue titled "Refund flow still broken
for Acme" is a sentence you would not want Acme to read, and a channel Buggie was
handed a URL for is a channel it cannot see the members of.

**Internal notes** follow the same switch. Unticked, a private note produces no message
at all, which is how webhooks behave. Ticked, the channel is told that somebody left an
