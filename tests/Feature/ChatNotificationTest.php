...
class ChatNotificationTest extends TestCase
{
    ...
    private function integration(array $state = []): ChatIntegration
    {
        return ChatIntegration::create([
            ...
            // The team's own channel, which is what these tests are about. A channel
            // with a client in it is the exception and the tests for it say so.
    ...
    #[Test]
    public function an_internal_issue_is_not_announced_in_a_clients_channel(): void
    {
        // The per-project filter invites pointing an integration at one client's own
        // Slack. An internal issue's title must not follow it there.
        [$workspace, $owner] = $this->workspaceWithMember(slug: 'acme');
        ...
        app(Tenancy::class)->run($workspace, function () use ($owner) {
            ...
            // A channel that is NOT the team's own.
            ChatIntegration::create([
                'provider' => ChatProvider::Slack->value,
                'name' => "The client's channel",
    ...
    #[Test]
    public function a_client_visible_issue_is_announced_there(): void
    {
        // The paired positive control. Without it the test above passes for a chat
        // integration that never sends anything at all.
        [$workspace, $owner] = $this->workspaceWithMember(slug: 'acme');
